...
def create_media_tools(client_phone: str, is_init_message: bool = False):
    ...
    async def show_product_photos(product_ids: List[int]) -> str:
        ...
        - При обычных запросах отправляется ТОЛЬКО 1 фото (первый товар из списка)
        ...
        Args:
            product_ids: Список ID товаров для отправки фото. ОБЯЗАТЕЛЬНО извлеки из секции [PRODUCT_IDS] ответа инструментов поиска (vector_search, execute_sql_request). НЕ используй ID из старых запросов.
