...
async def vector_search(query: str) -> str:
    ...
    ✅ Используй для:
    - Текстовые запросы о товарах: "Что у вас есть?", "Покажи мясо", "Какие стейки?"
    - Поиск по типу/части: "грудинка свиная", "говядина", "стейки", "полуфабрикаты"
    - Поиск по поставщику: "товары от Коралл", "продукция Мироторг", "что есть от X"
...
async def get_random_products(limit: int = 10) -> str:
    ...
    - Пользователь задал конкретный вопрос - сначала попробуй vector_search или execute_sql_request

    ════════════════════════════════════════════════════════════════════════════════
    ВАЖНО:
    ════════════════════════════════════════════════════════════════════════════════

    Это инструмент последней надежды! Всегда сначала пробуй vector_search или execute_sql_request.
    Используй get_random_products только когда все остальные методы поиска не дали результатов.
