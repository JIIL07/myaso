...
async def vector_search(query: str) -> str:
    """Семантический поиск товаров по текстовому запросу пользователя.

    Использует векторный поиск (embeddings) для нахождения релевантных товаров по смыслу запроса.
    Возвращает до 50 товаров в компактном формате с ID для последующей отправки фото.

    Args:
        query: Текстовый запрос пользователя о товарах (например, "грудинка свиная", "товары от Коралл")
...
async def get_random_products(limit: int = 10) -> str:
    ...
    Используется как последняя надежда, когда все остальные инструменты поиска не дали результатов.
    Возвращает случайные товары для демонстрации ассортимента.
    ...
    Returns:
        Строка с отформатированным списком случайных товаров и их ID в секции [PRODUCT_IDS]
    """
    if limit > 20:
        limit = 20
