...
1. vector_search(query: str, require_photo: bool = False) → str
  ...
  ПАРАМЕТРЫ:
    ...
    - require_photo: Если True, возвращает только товары с фотографиями (используй когда пользователь просит показать фото или нужны товары с фото)
...
2. generate_sql_from_text(text_conditions: str, topic: Optional[str] = None) → str
  ...
  НАЗНАЧЕНИЕ: Генерирует SQL запрос (WHERE условия или полный SELECT) из текстового описания на русском языке
  ...
  ВАЖНО: После вызова generate_sql_from_text ОБЯЗАТЕЛЬНО вызови execute_sql_query с полученным SQL запросом!
  ВОЗВРАЩАЕТ: SQL запрос (WHERE условия или полный SELECT) для использования в execute_sql_query

3. execute_sql_query(sql_query: str, limit: int = 50) → str
  ...
  ИСПОЛЬЗУЙ КОГДА:
    ...
    - У тебя есть полный SELECT запрос с JOIN/подзапросами
    - Нужно выполнить SQL запрос для поиска товаров

  ВАЖНО:
    - Можно использовать как с generate_sql_from_text, так и напрямую с полными запросами
    - Для WHERE условий: execute_sql_query(sql_query="supplier_name = 'ООО КИТ'")
    - Для полных запросов: execute_sql_query(sql_query="SELECT ... FROM myaso.products JOIN ...")
