...
import threading
from typing import Any, Dict, Literal, overload, Tuple, Type, TypeVar
...
class AgentFactory:
    ...
    def get_agent(self, name: str, config: Dict[str, Any], *, use_cache: bool = True) -> BaseAgent:
        ...
        Если use_cache=False, всегда создаёт новый экземпляр без сохранения в кэш.

        Args:
            name: Имя зарегистрированного агента
            config: Словарь конфигурации агента
            use_cache: Использовать ли кэш (по умолчанию True). Если False, всегда создаёт новый экземпляр.

        Returns:
            Экземпляр агента. Для известных агентов (например, "product") возвращает
            конкретный тип, иначе - базовый BaseAgent.
        ...
        # Если кэш отключен, всегда создаём новый экземпляр
        if not use_cache:
