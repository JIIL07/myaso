    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            update_root: Автоматически обновлять root trace с входными и выходными данными
        ...
        if settings.langfuse.langfuse_enabled and settings.langfuse.langfuse_public_key:
            try:
                ...
                # Параметр updateRoot не поддерживается в новых версиях LangFuse
                # Удаляем его из kwargs, если он был передан
                kwargs.pop("updateRoot", None)
                kwargs.pop("update_root", None)
                
                handler_kwargs.update(kwargs)
                
                self._langfuse_handler = LangfuseCallbackHandler(**handler_kwargs)
            except Exception as e:
                logger.warning(f"Не удалось инициализировать LangFuse handler: {e}", exc_info=True)
    ...
    def save_conversation_to_langfuse(self) -> None:
        ...
        if self._langfuse_client:
            try:
                ...
                logger.warning(f"Не удалось отправить данные в LangFuse через client: {e}", exc_info=True)
            ...
            try:
                ...
                logger.warning(f"Не удалось отправить данные в LangFuse через handler: {e}", exc_info=True)