...
4. ОБЩИЕ ПРАВИЛА:
   ...
   - Для текстовых поисков используй LIKE или ILIKE с правильным экранированием'
)
...
2. КАК ОТПРАВЛЯТЬ:
   - Используй инструмент show_product_photos с product_ids из секции [PRODUCT_IDS] результатов поиска
   - Извлекай product_ids из ответов инструментов vector_search, execute_sql_query, get_random_products
...
3. ИЗВЛЕЧЕНИЕ PRODUCT_IDS:
   - ВСЕГДА извлекай product_ids из секции [PRODUCT_IDS] ответов инструментов
   - Формат: [PRODUCT_IDS]{"product_ids": [1, 2, 3]}[/PRODUCT_IDS]
   - Используй эти ID для show_product_photos
